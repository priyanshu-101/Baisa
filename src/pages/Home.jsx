    ...
    return (
        <div className="w-full bg-gradient-to-b from-pink-50 to-white min-h-screen" data-aos="fade-up" data-aos-duration="600" data-aos-delay="400">
            ...
            <div className="w-full bg-gradient-to-b from-white to-pink-50 py-16">
                <div className="w-full px-6 md:px-12">
                    <div className="flex items-center justify-between mb-8 w-full mx-auto">
                        <h2 className="text-3xl md:text-4xl font-serif font-bold text-gray-800">Find the Perfect Piece  </h2>
                    </div>

                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 w-full mx-auto">
                        {perfectPiece.map((perfect) => (
                            <div key={perfect.id} className="group cursor-pointer">
                                <div className={`relative overflow-hidden rounded-lg bg-gradient-to-br ${perfect.gradient} aspect-[3/4] mb-4`}>
                                    <div className={`absolute inset-0 bg-gradient-to-br ${perfect.overlayGradient} opacity-80`}></div>
                                    <div className="absolute inset-0 flex items-center justify-center">
                                        <div className="text-center text-white">
                                            <div className="text-6xl mb-4">{perfect.icon}</div>
                                        </div>
                                    </div>
                                    <div className="absolute inset-0 bg-black opacity-0 group-hover:opacity-20 transition-opacity duration-300"></div>
                                </div>
                                <div className='flex items-center justify-between cursor-point'>
                                    <h3 className="text-lg font-medium text-gray-800 mb-2">{perfect.name}</h3>
                                    <ArrowRight className="w-5 h-5 text-pink-600 mb-2" />
                                </div>
                            </div>
                        ))}
                    </div>
